...
class abstr_inliner(core.module.Translator):
    ...
    def _inlineIfNeeded(self, stmt):
        ...
        if isinstance(stmt, pycparser.c_ast.Label):
            ...
            #TODO just to avoid troubles with a SVCOMP benchmark. You should implement ternary conditions properly, actually
            if type(stmt.stmt) is c_ast.TernaryOp and type(stmt.stmt.iftrue) is c_ast.FuncCall and type(stmt.stmt.iftrue.name) is c_ast.ID and stmt.stmt.iftrue.name.name == "reach_error":
                original = "assert(!("+self._generate_stmt(stmt.stmt.cond).strip()[:-1]+"));"
                original = label + ':;\n' + self.inlinedStack[-1] + original
            else:
                original = self._generate_stmt(stmt.stmt)
                # TODO this is a source of bugs: (a?b():0) is not correctly inlined, as b(); a?return_of_b:0 is not ok
            ...
            #TODO just to avoid troubles with a SVCOMP benchmark. You should implement ternary conditions properly, actually
            if type(stmt) is c_ast.TernaryOp and type(stmt.iftrue) is c_ast.FuncCall and type(stmt.iftrue.name) is c_ast.ID and stmt.iftrue.name.name == "reach_error":
                original = "assert(!("+self._generate_stmt(stmt.cond).strip()[:-1]+"));"
                original = self.inlinedStack[-1] + original
                #print(stmt, original)
            else:
                original = self._generate_stmt(stmt)
    ...
    def _inlineFunction(self, n, fcall_ast_node, simple):
        ...
        fInput = fOutput = ''
        fref = n.decl.name
        atomicFnc = fref.startswith('__CSEQ_atomic')
    ...
    def ____needsInlining(self, f):
        ...
        return (b or
                (f in self.Parser.funcName and  # defined functions need to be inlined when called (if at all)
                 #not f.startswith('__CSEQ_atomic') and  # G: atomic functions will be inlined 
