...
class lazyseqnewschedule(core.module.Translator):
	...
	def needs_compulsory_visible_point(self, n):
		return (type(n) == pycparser.c_ast.FuncCall and hasattr(n, "name") and hasattr(n.name, "name") and n.name.name in (core.common.changeID['pthread_join'],core.common.changeID['pthread_create'])) or (type(n) == pycparser.c_ast.Assignment and type(n.rvalue) == pycparser.c_ast.FuncCall and hasattr(n.rvalue, "name") and hasattr(n.rvalue.name, "name") and n.rvalue.name.name in (core.common.changeID['pthread_create'], core.common.changeID['pthread_join'])) or (type(n) == pycparser.c_ast.Assignment and type(n.rvalue) is pycparser.c_ast.BinaryOp and type(n.rvalue.right) == pycparser.c_ast.Assignment and type(n.rvalue.right.rvalue) == pycparser.c_ast.FuncCall and hasattr(n.rvalue.right.rvalue, "name") and hasattr(n.rvalue.right.rvalue.name, "name") and n.rvalue.right.rvalue.name.name in (core.common.changeID['pthread_create'], core.common.changeID['pthread_join'])) or (type(n) == pycparser.c_ast.Assignment and type(n.rvalue) is pycparser.c_ast.BinaryOp and type(n.rvalue.left) is pycparser.c_ast.FuncCall and n.rvalue.left.name.name in (core.common.changeID['pthread_create'], core.common.changeID['pthread_join']))
...
#@@@@		code = self.visit(stmt)
					...
					guard = ''
					if not self.__atomic and not self.am_main_nothing_concur:
						guard = '@£@G'
				...
				# Case 3: all the rest....
					...
					#      (now we know if the statement is accessing global memory,
					#       so to insert the stamp at the beginning when needed)
					if not self.__atomic and self.__stmtCount == -1 and not self.am_main_nothing_concur:   # first statement in a thread
						self.__stmtCount += 1
						self.__maxInCompound = self.__stmtCount
						threadIndex = self.Parser.threadOccurenceIndex[self.__currentThread]
						s =  self.visit(stmt)
	...
	def visit_FuncDef(self, n):
		...
		decl = self.visit(n.decl)
		self.indent_level = 0
		self.am_main_nothing_concur = self.__currentThread == 'main'
		body = self.visit(n.body)
		self.am_main_nothing_concur = False

		f = ''

		self.__lines[self.__currentThread] = self.__stmtCount - (self.main_skip_first if self.__currentThread == 'main' else 0)
		...
		if self.__currentThread == 'main':
			f = '%s main_thread(_Bool firstrun)\n' % self.Parser.funcBlockOut[
				self.__currentThread]
			...
			if main_args.find('void') != -1 or main_args == '':
				...
				args += main_args[1] + ';'
			body = '{\nif(!firstrun) goto tmain_thread_1;\n' + args + body[body.find('{') + 1:]
...
#			fName = fName.replace('&', '')
		...
		if ((fref == core.common.changeID['pthread_mutex_lock'] ) or (fref == core.common.changeID['pthread_mutex_unlock']) or
				fref.startswith('__cs_cond_wait_') or (fref == core.common.changeID['pthread_mutex_trylock'])):
			#threadIndex = self.Parser.threadIndex[self.__currentThread] if self.__currentThread in self.Parser.threadIndex else 0
			threadIndex= self.Parser.threadOccurenceIndex[self.__currentThread] if self.__currentThread in self.Parser.threadOccurenceIndex else 0 # 17 March 2021
			self.addRetFuncCall(fref,args,threadIndex)
			return fref + '(' + args + ', %s)' % threadIndex
