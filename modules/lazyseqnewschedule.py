...
class lazyseqnewschedule(core.module.Translator):
	...
	def needs_compulsory_visible_point(self, n):
		return (type(n) == pycparser.c_ast.FuncCall and n.name.name in (core.common.changeID['pthread_join'],core.common.changeID['pthread_create'])) or (type(n) == pycparser.c_ast.Assignment and type(n.rvalue) == pycparser.c_ast.FuncCall and n.rvalue.name.name in (core.common.changeID['pthread_create'], core.common.changeID['pthread_join'])) or (type(n) == pycparser.c_ast.Assignment and type(n.rvalue) is pycparser.c_ast.BinaryOp and type(n.rvalue.right) == pycparser.c_ast.Assignment and type(n.rvalue.right.rvalue) == pycparser.c_ast.FuncCall and n.rvalue.right.rvalue.name.name in (core.common.changeID['pthread_create'], core.common.changeID['pthread_join']))
