...
class unroller(core.module.Translator):
    ...
    def loadfromstring(self, string, env, fill_only_fields=None):
        ...
        if 'unwind-for' not in env.paramvalues:
            self.forunwind = self.unwind
        else:
            self.forunwind = int(env.paramvalues['unwind-for'])

        if 'unwind-for-max' not in env.paramvalues:
            self.formaxunwind = None
        else:
            self.formaxunwind = int(env.paramvalues['unwind-for-max'])

        if 'unwind-while' not in env.paramvalues:
            self.whileunwind = self.unwind
        else:
            self.whileunwind = int(env.paramvalues['unwind-while'])

        if 'softunwindbound' not in env.paramvalues:
            self.softunwindbound = False
        else:
            self.softunwindbound = True
    ...
    def _calculateLoopBasevalue(self, n):
        if self._loopIsBounded(n):
            return int(n.init.rvalue.value)
        else:
            return None
    ...
    def visit_For(self, n):
        ...
        for i in range(0, bound):
            ...
            #
            if i == 0:
                s += self._make_indent()  # ~+ '/*  - - - - > loop %s, iter = 0 */\n' % currentLoopID
            else:
    ...
    def visit_FuncDef(self, n):
        ...
        # print("FUN STARTS: " + str(datetime.datetime.now().time()))
        decl = self.visit(n.decl)
        self.indent_level = 0
        self.__seen_labels = set()
        body = self.visit(n.body)

        # print("FUN ENDS: " + str(datetime.datetime.now().time()))
        if n.param_decls:
    ...
    def visit_Label(self, n):
        ...
        if self.__loopDepth > 0 and not self.__visitingGoto:
            ...
            newLabel = oldLabel = n.name
            # S:the following has been added to avoid changing the label in the first  iteration, it could be pointed to from outside loop
            if self.__labelCount[n.name] > 0:
                newLabel = n.name + '_%s' % self.__labelCount[n.name]
            # S

            self.__labelNew[oldLabel] = newLabel
            self.__labelToChange[-1][n.name] = True
            #self.__seen_labels.add(newLabel)

            return newLabel + ':\n' + self._generate_stmt(n.stmt)
        else:
            #self.__seen_labels.add(n.name)
            return n.name + ':\n' + self._generate_stmt(n.stmt)

    def visit_Goto(self, n):
        self.__visitingGoto = True
        #assert n.name not in self.__seen_labels
        s = 'goto %s;' % n.name
