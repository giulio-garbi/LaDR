    ...
    if isPreprocessed(filename):
        ...
        if fileStartsWith(filename, '# '):
            ...
            for i in range(len(allLines)):
                ...
                for j in range(i, len(allLines)):
                    if allLines[j].startswith('extern ') and not allLines[j].startswith("extern void __VERIFIER_atomic_begin(void);") and not allLines[j].startswith("extern void __VERIFIER_atomic_end(void);"):
...
def shorthashstring(file):
    return hashlib.md5(str).hexdigest()[0:4].upper()
...
def parseConfig(inputfile):
    try:
        ...
        config = ijson.parse(input)
        # return fd to allow closing it at the end
        return config, input
    except IOError as e:
