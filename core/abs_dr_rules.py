...
from pycparser.c_ast import BinaryOp, Assignment
...
class AbsDrRules:            
    ...
    def getsm(self, addr, field):
        if field.startswith("dr") and ("_cs_tmp_if_cond_" in addr or "_cs_tmp_while_cond_" in addr or "_cs_tmp_for_cond_" in addr):
            return "0"
        else:
    ...
    def __arrayref_bavtmp_dr(self, state, dr_mode, postExp, exp, **kwargs):
        ...
        assert(self.dr_on)
        ok = lambda state: self.and_expr(self.p2code(self.getVpstate(**kwargs)), self.brackets(self.assign_with_prop(state, "dr",  self.or_expr_prop( 
            self.getsm("&("+self.brackets(self.visitor_visit(state, postExp, "VALUE", "WSE", **kwargs))+"["+self.visitor_visit(state, exp, "VALUE", "WSE", **kwargs)+"])", self.getsm_dr(kwargs)),self.cp(state, "dr"), self.cp(state, "wam")))))
    ...
    def __structrefptr_bavtmp_dr(self, state, dr_mode, postExp, exp, **kwargs):
        ...
        assert(self.dr_on)
        ok = lambda state: self.and_expr(self.p2code(self.getVpstate(**kwargs)), self.brackets(self.assign_with_prop(state, "dr", self.or_expr_prop( 
            self.getsm("&("+self.brackets(self.visitor_visit(state, postExp, "VALUE", "WSE", **kwargs))+"->"+exp.name+")", self.getsm_dr(kwargs)),self.cp(state, "dr"), self.cp(state, "wam")))))
        err = lambda state: self.and_expr(self.p2code(self.getVpstate(**kwargs)), self.brackets(self.assign_with_prop(state, "dr", self.or_expr_prop(self.cp(state, "dr"), self.cp(state, "wam"), self.cp(state, "wkm")))))
        if dr_mode in ("NO_ACCESS", "PREFIX"):
            return ""
        elif self.abs_on:
    ...
    def __structrefvar_bal_dr(self, state, dr_mode, postExp, exp, **kwargs):
        ...
        assert(self.dr_on)
        ok = lambda state: self.and_expr(self.p2code(self.getVpstate(**kwargs)), self.brackets(self.assign_with_prop(state, "dr", self.or_expr_prop( 
            self.getsm("&("+self.brackets(self.visitor_visit(state, postExp, "LVALUE", "WSE", **kwargs))+"."+exp.name+")", self.getsm_dr(kwargs)),self.cp(state, "dr"), self.cp(state, "wam")))))
        err = lambda state: self.and_expr(self.p2code(self.getVpstate(**kwargs)), self.brackets(self.assign_with_prop(state, "dr", self.or_expr_prop(self.cp(state, "dr"), self.cp(state, "wam"), self.cp(state, "wkm")))))
        if dr_mode in ("NO_ACCESS", "PREFIX"):
            return ""
        elif self.abs_on:
    ...
    def __ptrop_dr(self, state, dr_mode, castExp, **kwargs):
        ...
        assert(self.dr_on)
        ok = lambda state: self.and_expr(self.p2code(self.getVpstate(**kwargs)), self.brackets(self.assign_with_prop(state, "dr", self.or_expr_prop( 
            self.getsm("("+self.visitor_visit(state, castExp, "VALUE", "WSE", **kwargs)+")", self.getsm_dr(kwargs)),self.cp(state, "dr"), self.cp(state, "wam")))))
        err = lambda state: self.and_expr(self.p2code(self.getVpstate(**kwargs)), self.brackets(self.assign_with_prop(state, "dr", self.or_expr_prop(self.cp(state, "dr"), self.cp(state, "wam"), self.cp(state, "wkm")))))
        if dr_mode in ("NO_ACCESS", "PREFIX"):
            return ""
        elif self.abs_on:
    ...
    def rule_Assert_Assume(self, state, fullExpr, abs_mode, dr_mode, full_statement, **kwargs):
        ...
        if fncName in ("__CSEQ_assert", "assert", "__CPROVER_assert", "__assert_fail"):
            if  self.dr_possible:
    ...
    def rule_BinaryOp(self, state, fullOp, abs_mode, dr_mode, full_statement, **kwargs):
        ...
        if abs_mode in ("VALUE", None) and dr_mode in ("WSE",None):
            if type(exp2) is Assignment:
                ans = "(("+self.visitor_visit(state, exp1, "VALUE", "WSE", **kwargs)+") "+op+" ("+self.rule_Assignment(state, exp2, "VALUE", "WSE", False, **kwargs)+"))"
            else:
    ...
    def __assignment_manual_cp_p2(self, state, unExpr, **kwargs):
        return self.and_expr(self.p2code(self.getVpstate(**kwargs)),
            self.brackets(self.assign(state, "dr", self.or_expr_prop(self.getsm("&("+self.visitor_visit(state, unExpr, "LVALUE", "WSE", **kwargs)+")", self.getsm_dr(kwargs)), self.cp(state, "dr"), self.cp(state, "wam"), 
                )))
            )
